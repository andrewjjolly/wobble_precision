...
rvs = np.array(results['RV'] - np.median(results['pipeline_rv'])) #pipeline RVs from the HARPS pipeline
rvs_err = np.array(results['RV_err']) #pipeline RV errors
star_name = 'GL667c (via wobble)'
...
with pm.Model() as model:
    ...
    t0 = pm.Uniform("t0", lower = t0s - period, upper = t0s + period,  shape=1, testval = t0s)
    logP = pm.Uniform(
        "logP",
...
"""
fitting for the maximum a posterior parameters.
"""
with model:
...
map_period = np.exp(map_soln['logP'])
map_t0 = map_soln['t0']
map_k = np.exp(map_soln['logK'])
map_ecc = map_soln['ecc']
...
with model:
    _ = corner.corner(trace, var_names=["P", "logK", "ecc", "omega", "t0"])
...
plt.scatter(phased_time, residuals)
...
times_large_sig = times[residuals_abs > 2*sigma]
rvs_large_sig = data[residuals_abs > 2*sigma]
